...
public:
	MMN();
	MMN(int N, int M, TVector<double> XBorder, TVector<double> YBorder); //Конструктор инициализации
...
public:
	void SetParams();
	double Runner();
	TVector<double> SolvingTestTask(TVector<double> eps, TVector<int> MaxIterations, char Name);
};
